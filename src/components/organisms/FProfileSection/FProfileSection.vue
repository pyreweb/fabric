  ...
  <f-card :bordered="bordered">
    ...
    <div v-if="!isLoading">
      ...
      <f-form v-if="isEditing" @submit="handleSubmit">
        ...
        <!-- Form fields slot -->
        <slot name="edit-fields" :formData="localFormData" :errors="validationErrors">
          <!-- Default field rendering if fields prop is provided -->
          <div class="flex flex-col gap-4">
            <template v-for="field in fields">
              <div v-if="field.type === 'textarea'" :key="field.name" class="flex flex-col gap-1.5">
                <label
                  v-if="field.label"
                  :for="`field-${field.name}`"
                  :class="['text-sm font-medium text-gray-700', { 'after:content-[\'_*\'] after:text-red-500': field.required }]"
                >
                  {{ field.label }}
                </label>
                <f-textarea
